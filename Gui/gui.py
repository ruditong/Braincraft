...
class MainWindow(QMainWindow):
    ...
    #     # TEST
    #     self.relay_thread = RelayThread(channels=[23, 24])
...
class GUI(QWidget):
    ...
    def _close_window(self, event, window):
        event.accept()
        if isinstance(window, TriggerWindow): self.triggerWindow = None
    ...
    def _trigger_button(self):
        ...
        if self.triggerWindow is None:
            ...
            self.triggerWindow.closeEvent = lambda event: self._close_window(event, self.triggerWindow)
    ...
    def _trigger_submit(self, close=False):
        ...
        # Get parameter and update
        if close:
            self.triggerWindow.setWindowModality(QtCore.Qt.NonModal)
            self.triggerWindow.close()
            self.triggerWindow = None
            return
    ...
    def _update_button(self):
        ...
        if len(self.buffers) == 0: return 
        id = self.info.parameters.pageCombo.currentIndex()
        self.info.parameters.pageCombo.setItemText(id, self.info.parameters.stackedLayout.widget(id).outputs['name'].text())
...
class DynamicPlotter(pg.GraphicsLayoutWidget):
    ...
    def setYrange(self):
        ...
        ticks = np.arange(-1, 2*(ncurves-1)+2.5)[1::2]
        self.plot.setYRange(-1, 2*(ncurves-1)+2.5, padding=0)
...
class ConnectionPopupWindow(QWidget):
    ...
    def __init__(self, weight, delay, start, end):
        ...
        self.label = QLabel(f"Connection {start} to {end}")
        ...
        pagelayout.addRow(f"Delay (s)", self.delay)
...
class ConnectionWindow(QWidget):
    ...
    def __init__(self, id=None):
        ...
        self.headers.addWidget(QLabel("Delay (s)"))
...
#             isOutput = GPIO.gpio_function(channel) == GPIO.OUT
#             GPIO.setup(channel, GPIO.IN)
#             GPIO.add_event_detect(channel, GPIO.RISING, callback=self.queue.put)
#             if isOutput: GPIO.setup(channel, GPIO.OUT)

#     def run(self):
#         while True: self.event_detected.emit(self.queue.get())
